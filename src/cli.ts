...
import { ProcessManager, type ProcessData } from "./process-manager.js";
...
export class ProcessManagerCLI {
  ...
  private setupCommands() {
    ...
    this.program
      ...
      .action(async (pidStr: string, options: CliOptions) => {
        await this.logsCommand(parseInt(pidStr, 10), options);
      });
  }

  private async startCommand(command: string, options: CliOptions) {
  ...
  private async listCommand(showAll: boolean = false) {
    let processes: { [key: string]: ProcessData };
  ...
  private async stopCommand(pid: number) {
  ...
  private async logsCommand(pid: number, options: CliOptions) {
    if (isNaN(pid)) {
